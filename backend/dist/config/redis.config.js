 ...
 */
export function isRedisPermanentlyDisabled() {
 ...
 */
export async function executeRedisOperation(operation, fallbackValue, operationName = 'Redis operation') {
    if (!isRedisAvailable()) {
 ...
 * @param operationName Name of operation for logging
 */
export async function executeRedisVoidOperation(operation, operationName = 'Redis operation') {
    if (!isRedisAvailable()) {
