 ...
 */
export function recordEINAccess(userId, action, req, churchId) {
    const record = {
        userId,
        timestamp: new Date(),
        action,
 ...
 */
async function checkForAnomalies(userId, req) {
    const now = new Date();
    const oneHourAgo = new Date(now.getTime() - 60 * 60 * 1000);
    const oneDayAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000);
    // Get recent access by this user
    const accessInLastHour = accessLog.filter(record => record.userId === userId && record.timestamp >= oneHourAgo);
    const accessInLastDay = accessLog.filter(record => record.userId === userId && record.timestamp >= oneDayAgo);
 ...
 */
function getUserTypicalAccessHours(userId) {
 ...
 */
function getUserPreviousIPs(userId) {
    const userAccess = accessLog.filter(record => record.userId === userId);
 ...
 */
export function securityMonitoring(operationType) {
    return (req, res, next) => {
        const userId = req.user?.adminId || 'unknown';
 ...
 */
export async function getSecurityAlerts(timeframe = '24h') {
 ...
 */
export async function getUserAccessStats(userId) {
    const now = new Date();
    const oneDayAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000);
    const oneWeekAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
    const accessToday = accessLog.filter(record => record.userId === userId && record.timestamp >= oneDayAgo);
    const accessThisWeek = accessLog.filter(record => record.userId === userId && record.timestamp >= oneWeekAgo);
    return {
        userId,
