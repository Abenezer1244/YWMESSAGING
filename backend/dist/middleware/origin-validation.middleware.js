...
function validateOrigin(origin) {
    ...
    const isAllowed = ALLOWED_ORIGINS.some(allowedOrigin => {
        ...
        if (origin.replace('http://', 'https://') === allowedOrigin)
            return true;
        if (origin.replace('https://', 'http://') === allowedOrigin.replace('https://', 'http://'))
            return true;
        return false;
    });
 ...
 */
export async function reportCSRFAttempt(req, origin, referer) {
    const csrfEvent = {
        type: 'CSRF_ATTEMPT',
...
export function getAllowedOrigins() {
    return ALLOWED_ORIGINS.filter(Boolean);
}
export default {
...
//# sourceMappingURL=origin-validation.middleware.js.map