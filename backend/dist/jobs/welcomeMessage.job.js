 ...
 * Delay: 1 minute
 */
export async function sendWelcomeMessage(groupMemberId, groupId, memberId) {
    try {
        ...
        console.log(`Welcome message sent to ${member.phone} for group ${group.name}`);
    }
    catch (error) {
 ...
 */
export async function queueWelcomeMessage(groupMemberId, groupId, memberId, delayMs = 60000 // 1 minute default
) {
    ...
    setTimeout(() => {
        sendWelcomeMessage(groupMemberId, groupId, memberId).catch((error) => console.error('Welcome message job failed:', error));
    }, delayMs);
}
