...
export async function requestAccountDeletion(req, res) {
    try {
        ...
        const result = await gdprService.requestAccountDeletion(churchId, adminId, reason);
        // TODO: Send confirmation email with token
        console.log(`Deletion request for church ${churchId}. Token: ${result.confirmationToken}`);
        res.json({
            success: true,
            data: {
...
export async function confirmAccountDeletion(req, res) {
    try {
        ...
        const result = await gdprService.confirmAccountDeletion(churchId, confirmationToken);
        res.json({
            success: true,
            data: result,
        });
...
export async function updateConsent(req, res) {
    try {
        ...
        const result = await gdprService.updateConsent(churchId, type, status, reason);
        res.json({
            success: true,
            data: result,
        });
 ...
 */
export async function getConsentHistory(req, res) {
    try {
        const { churchId } = req.user || {};
        if (!churchId) {
            return res.status(401).json({
        ...
        const history = await gdprService.getConsentHistory(churchId, type);
        res.json({
            success: true,
            data: {
                churchId,
