 ...
 */
export async function getPlanningCenterStatus(req, res) {
    try {
        ...
        res.status(500).json({
            success: false,
            error: error.message || 'Failed to get Planning Center status',
        });
    }
}
/**
 ...
 */
export async function connectPlanningCenter(req, res) {
    try {
        const churchId = req.user?.churchId;
        if (!churchId) {
            return res.status(401).json({
        ...
        res.status(400).json({
            success: false,
            error: error.message || 'Failed to connect Planning Center',
        });
    }
}
/**
 ...
 */
export async function syncPlanningCenterMembers(req, res) {
    try {
        const churchId = req.user?.churchId;
        if (!churchId) {
            return res.status(401).json({
        ...
        res.status(500).json({
            success: false,
            error: error.message || 'Failed to sync members',
        });
    }
}
/**
 ...
 */
export async function disconnectPlanningCenter(req, res) {
    try {
        const churchId = req.user?.churchId;
        if (!churchId) {
            return res.status(401).json({
        ...
        res.status(500).json({
            success: false,
            error: error.message || 'Failed to disconnect Planning Center',
        });
    }
}
/**
 ...
 */
export async function validatePlanningCenterConnection(req, res) {
    try {
        const churchId = req.user?.churchId;
        if (!churchId) {
            return res.status(401).json({
...
//# sourceMappingURL=planning-center.controller.js.map