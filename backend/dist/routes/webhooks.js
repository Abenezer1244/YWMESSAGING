...
function verifyTelnyxWebhookSignature(payload, signatureHeader, timestampHeader, publicKeyBase64) {
    ...
    try {
        ...
        const publicKey = crypto.createPublicKey({
            ...
            format: 'raw',
            type: 'ed25519',
        }); // Using 'any' to avoid TypeScript strictness for older Node.js versions
        ...
        const isValid = crypto.verify(null, // algorithm is null for ED25519
        Buffer.from(signedMessage, 'utf-8'), publicKey, signatureBuffer);
        if (!isValid) {
        ...
        if (timeDifferenceSeconds > MAX_AGE_SECONDS) {
            console.warn(`⚠️ Webhook timestamp is ${timeDifferenceSeconds}s old (max: ${MAX_AGE_SECONDS}s)`);
 ...
 */
router.post('/10dlc/status', async (req, res) => {
    try {
        // Get headers
 ...
 */
router.post('/10dlc/status-failover', async (req, res) => {
    try {
        const signature = req.headers['telnyx-signature-ed25519'];
        const timestamp = req.headers['telnyx-timestamp'];
        const payload = req.body;
 ...
 */
router.get('/10dlc/status', (req, res) => {
