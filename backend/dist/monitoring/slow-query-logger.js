 ...
 */
export function initializeSlowQueryLogging(prisma) {
    ...
    prisma.$use(async (params, next) => {
        ...
        try {
            ...
            if (newrelic) {
                ...
                newrelic.recordMetric('Custom/Database/Query/Error/Latency', duration);
            }
 ...
 */
function recordQueryMetric(params, duration) {
    if (!newrelic)
        return;
    ...
    // Per-model metrics
    if (params.model) {
        newrelic.recordMetric(`Custom/Database/Model/${params.model}/Latency`, duration);
    }
    // Per-action metrics (find, create, update, delete, etc.)
    if (params.action) {
        newrelic.recordMetric(`Custom/Database/Action/${params.action}/Latency`, duration);
    }
    // Per-model-action metrics
    if (params.model && params.action) {
        newrelic.recordMetric(`Custom/Database/${params.model}/${params.action}/Latency`, duration);
    }
}
/**
 * Log slow query with detailed analysis
 */
function logSlowQuery(event, isCritical = false) {
    const logLevel = isCritical ? 'error' : 'warn';
    const logFn = isCritical ? console.error : console.warn;
    logFn(`[SLOW QUERY] ${event.operation} took ${event.duration}ms (threshold: 500ms)`);
    if (isCritical) {
    ...
    if (newrelic) {
        ...
        if (isCritical) {
            ...
            if (newrelic.noticeError) {
                newrelic.noticeError(new Error(`Critical slow query: ${event.operation} (${event.duration}ms)`));
            }
        }
    }
...
export function printSlowQueryReport() {
    ...
    Object.entries(report.byModel).forEach(([model, queries]) => {
        const avgDuration = Math.round(queries.reduce((sum, q) => sum + q.duration, 0) / queries.length);
    ...
    report.recentQueries.slice(-5).forEach((q) => {
        console.log(`  ${q.operation} - ${q.duration}ms - ${q.timestamp.toLocaleTimeString()}`);
    });
    console.log('\n═══════════════════════════════════════════════════════\n');
}
/**
 ...
 */
export function slowQueryReportHandler(req, res) {
 ...
 */
export function slowQueryMetricsMiddleware(req, res) {
    const stats = slowQueryLog.getStats();
...
export function collectSlowQueryMetrics() {
    ...
    if (newrelic) {
        ...
        newrelic.recordMetric('Custom/Database/CriticalSlowQuery/Total24h', criticalQueries);
        newrelic.recordMetric('Custom/Database/Query/AverageDuration', stats.avgDuration);
    }
}
/**
 ...
 */
export function startSlowQueryMetricsCollection(intervalMinutes = 30) {
    // Collect every N minutes
    setInterval(() => {
        collectSlowQueryMetrics();
        console.log('[SlowQueryLogger] Metrics collected and sent to New Relic');
    }, intervalMinutes * 60 * 1000);
    console.log(`[SlowQueryLogger] Slow query metrics collection started (every ${intervalMinutes} minutes)`);
}
//# sourceMappingURL=slow-query-logger.js.map