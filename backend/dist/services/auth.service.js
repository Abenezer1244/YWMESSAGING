...
import { provisionTenantDatabase, runTenantMigrations, deleteTenantDatabase } from './database-provisioning.service.js';
import { createCustomSpan, createDatabaseSpan, createExternalApiSpan } from '../utils/apm-instrumentation.js';
...
export async function registerChurch(input) {
    ...
    try {
        ...
        const stripeCustomerId = await createExternalApiSpan('stripe', 'customer.create', () => createCustomer(input.email, input.churchName), { email: input.email, churchName: input.churchName });
        // ============================================
...
async function loginInternal(input) {
    return createCustomSpan('admin.login', async () => {
        ...
        const church = await createDatabaseSpan('SELECT', 'church', () => registryPrisma.church.findFirst({
            where: { email: input.email },
        }), { email: input.email });
        if (!church) {
        ...
        const admin = await createDatabaseSpan('SELECT', 'admin', () => tenantPrisma.admin.findFirst({
            where: {
                email: input.email,
            },
