 ...
 */
export async function downloadAndUploadMedia(telnyxMediaUrl, conversationId, fileName) {
    ...
    try {
        ...
        // 5. Validate size
        const maxSize = parseInt(process.env.MAX_MEDIA_SIZE_BYTES || '524288000'); // 500MB default
        if (buffer.length > maxSize) {
            throw new Error(`File too large: ${(buffer.length / 1024 / 1024).toFixed(1)}MB (max ${(maxSize /
                1024 /
                1024).toFixed(1)}MB)`);
        }
 ...
 * NO COMPRESSION - stores full quality
 */
export async function uploadMediaFromFile(filePath, conversationId, fileName, mimeType) {
    try {
        // 1. Read file
 ...
 * - MIME type
 */
async function extractMediaMetadata(filePath, mimeType) {
    const buffer = fs.readFileSync(filePath);
    ...
    if (type === 'image') {
        try {
            ...
            console.log(`  📷 Image: ${imgMetadata.width}x${imgMetadata.height} (${imgMetadata.format})`);
        }
        catch (e) {
    ...
    if (type === 'video' || type === 'audio') {
        try {
            ...
            if (!isNaN(duration)) {
                ...
                console.log(`  🎬 ${type}: ${mins}:${secs.toString().padStart(2, '0')}`);
            }
        }
        catch (e) {
            console.warn(`  ⚠️ Could not extract ${type} duration (ffprobe may not be installed)`);
        }
    }
    return metadata;
}
 ...
 */
export async function deleteMedia(s3Key) {
    try {
 ...
 */
export async function getPresignedUrl(s3Key, expirationSeconds = 604800) {
 ...
 */
export async function getMediaInfo(s3Key) {
    try {
 ...
 */
export async function deleteOldMedia(retentionDays = 365) {
    ...
    try {
        ...
        do {
            ...
            const oldObjects = listResult.Contents.filter((obj) => obj.LastModified && obj.LastModified < cutoffDate);
 ...
 */
export async function validateMediaFile(filePath, mimeType) {
    const maxSize = parseInt(process.env.MAX_MEDIA_SIZE_BYTES || '524288000');
    const stats = fs.statSync(filePath);
    if (stats.size > maxSize) {
        return {
            valid: false,
            error: `File too large. Max ${(maxSize / 1024 / 1024).toFixed(1)}MB, got ${(stats.size /
                1024 /
                1024).toFixed(1)}MB`,
        };
    }
...
//# sourceMappingURL=s3-media.service.js.map