 ...
 */
export async function submitNPSSurvey(churchId, responderId, input) {
    // Validate score
 ...
 */
export async function getNPSAnalytics(churchId, daysBack = 30) {
    // Try cache first
    ...
    // Calculate NPS score
    const npsScore = Math.round(((scoreDistribution.promoters - scoreDistribution.detractors) /
        surveys.length) *
        100);
...
export async function getNPSByCategory(churchId, daysBack = 30) {
    ...
    surveys.forEach((survey) => {
        ...
        if (count > 0) {
            result[survey.category] = Math.round((categoryStats[survey.category] / count) * 10) / 10;
        }
    });
    return result;
}
 ...
 */
function detectSentiment(feedback) {
 ...
 */
export async function invalidateNPSCache(churchId) {
 ...
 */
export async function sendFollowupEmail(churchId, surveyId, email, message) {
