...
export async function verifyAndRecoverPhoneLinkings() {
    ...
    try {
        ...
        console.log(`${LINKING_RECOVERY_CONFIG.LOG_TAG} Found ${churchesNeedingRecovery.length} churches needing recovery`);
        ...
        console.log(`${LINKING_RECOVERY_CONFIG.LOG_TAG} ${churchesToRetry.length} churches ready for retry (respecting backoff)`);
        ...
        for (const church of churchesToRetry) {
            try {
                ...
                console.log(`${LINKING_RECOVERY_CONFIG.LOG_TAG} Retrying link for church ${church.id} (${church.name}): ${church.telnyxPhoneNumber}`);
                // Attempt to link
                const linkingResult = await linkPhoneNumberToMessagingProfile(church.telnyxPhoneNumber, church.telnyxWebhookId, church.id);
                ...
                if (linkingResult.success) {
                    console.log(`${LINKING_RECOVERY_CONFIG.LOG_TAG} ✅ Successfully recovered linking for church ${church.id} via ${linkingResult.method} method`);
                }
                else {
                    console.error(`${LINKING_RECOVERY_CONFIG.LOG_TAG} ❌ Recovery failed for church ${church.id}: ${linkingResult.error?.code} - ${linkingResult.error?.message}`);
                }
                // Step 5: Alert support if max retries exceeded
                if (!linkingResult.success && newRetryCount >= LINKING_RECOVERY_CONFIG.MAX_RETRIES) {
                    console.error(`${LINKING_RECOVERY_CONFIG.LOG_TAG} 🚨 ALERT: Church ${church.id} (${church.name}) has exceeded max linking retries. Manual intervention required.`);
                    // TODO: Send email to support team
                    // await sendSupportAlert(church, linkingResult);
                }
            }
            catch (churchError) {
                console.error(`${LINKING_RECOVERY_CONFIG.LOG_TAG} Error processing church ${church.id}: ${churchError.message}`);
                results.push({
                    churchId: church.id,
        ...
        console.log(`${LINKING_RECOVERY_CONFIG.LOG_TAG} Recovery job completed. Processed: ${results.length} churches. Duration: ${duration}ms`);
...
//# sourceMappingURL=phone-linking-recovery.service.js.map