 ...
 */
export function prefixCacheKey(namespace, id, suffix) {
    if (suffix) {
...
export async function withCache(key, fetcher, options = {}) {
    ...
    try {
        ...
        const cached = await getCachedValue(fullKey, compress);
        if (cached !== null) {
        ...
        logger.error('Cache-aside pattern failed', error, {
            key: fullKey,
 ...
 */
export function createCachedFunction(fetcher, keyBuilder, options = {}) {
    return async (...args) => {
 ...
 * // Later: cleanup();
 */
export function setupCacheWarming(key, fetcher, options = {}) {
    ...
    const intervalId = setInterval(async () => {
        try {
            ...
            logger.warn(`Cache warming failed for ${fullKey}`, error);
        }
    }, interval);
    // Warm immediately
    fetcher().then((data) => setCachedValue(fullKey, data, ttl, false)).catch((error) => {
        logger.warn(`Initial cache warming failed for ${fullKey}`, error);
    });
 ...
 * ]);
 */
export async function cascadeInvalidate(key, dependentKeys = []) {
    ...
    try {
        ...
        logger.warn(`Cascade invalidation failed`, { key }, undefined, error);
    }
}
/**
 ...
 */
export async function broadcastInvalidate(pattern, channel = 'cache:invalidated') {
    try {
        if (!redisClient.isOpen) {
            return;
        }
        ...
        logger.warn(`Broadcast invalidation failed`, error);
    }
}
/**
 ...
 * );
 */
export async function subscribeCacheInvalidation(channel, handler) {
    try {
        ...
        await subscriber.subscribe(channel, async (message) => {
            try {
                ...
                logger.error(`Cache invalidation handler failed`, error, {
                    pattern: message,
                    channel,
                });
            }
...
export async function getCacheStatistics() {
    ...
    try {
        ...
        logger.debug('Failed to get Redis memory info', { error: error.message });
    }
    return {
        hits: totalHits,
        misses: totalMisses,
 ...
 */
async function getCachedValue(key, compress = false) {
    try {
        if (!redisClient.isOpen) {
            return null;
        }
        ...
        logger.debug(`Failed to get cached value`, { key, error: error.message });
        return null;
    }
}
/**
 ...
 */
async function setCachedValue(key, value, ttl, compress = false) {
    try {
        if (!redisClient.isOpen) {
            return false;
        }
        ...
        logger.debug(`Failed to set cached value`, { key, error: error.message });
        return false;
    }
}
/**
 ...
 */
async function setupInvalidationTriggers(key, triggers) {
...
//# sourceMappingURL=redis-cache-optimization.js.map