 ...
 * ```
 */
export function initializeQueryMonitoring(prisma) {
    ...
    prisma.$use(async (params, next) => {
        ...
        if (duration > SLOW_QUERY_THRESHOLD_MS * 2) {
            logger.error(`Very slow query detected (${duration}ms)`, new Error(`Query exceeded double threshold: ${duration}ms > ${SLOW_QUERY_THRESHOLD_MS * 2}ms`), {
                operation: params.action,
                model: params.model,
                duration,
            });
 ...
 */
export function getSlowQueryPercentage() {
