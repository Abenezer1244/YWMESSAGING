 ...
 */
export function validateMemberRow(row, rowNumber) {
    const errors = [];
 ...
 */
export function formatAndValidate(rows) {
    const result = {
        valid: [],
        invalid: [],
    };
    rows.forEach((row, index) => {
        ...
        try {
            ...
            result.invalid.push({
                row: index + 2,
                data: row,
                errors: [`Failed to format: ${error.message}`],
            });
        }
    });
    return result;
...
//# sourceMappingURL=csvParser.util.js.map