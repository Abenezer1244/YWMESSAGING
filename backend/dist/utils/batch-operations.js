 ...
 */
export async function batchCreate(model, data, config = {}) {
    const { chunkSize = DEFAULT_CHUNK_SIZE, useCircuitBreaker = false, isolationLevel = 'READ_COMMITTED', shouldLog = true, ignoreErrors = false, } = config;
 ...
 */
export async function batchDelete(model, whereConditions, config = {}) {
    const { chunkSize = DEFAULT_CHUNK_SIZE, useCircuitBreaker = false, isolationLevel = 'READ_COMMITTED', shouldLog = true, ignoreErrors = false, } = config;
    const startTime = Date.now();
    const result = {
        successful: 0,
 ...
 */
export async function batchUpdate(model, data, config = {}) {
    const { chunkSize = DEFAULT_CHUNK_SIZE, useCircuitBreaker = false, isolationLevel = 'READ_COMMITTED', shouldLog = true, ignoreErrors = false, } = config;
    const startTime = Date.now();
    const result = {
        successful: 0,
 ...
 * );
 */
export async function parallelBatchProcess(items, processor, config = {}) {
 ...
 * );
 */
export async function streamBatchProcess(queryFn, processor, config = {}) {
