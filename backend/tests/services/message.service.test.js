...
describe('Message Service - Unit Tests', () => {
    ...
    describe('createMessage()', () => {
        ...
        test('✅ Should reject message with no recipients', async () => {
            await expect(messageService.createMessage(testChurch.id, {
        ...
        test('✅ Should batch create recipient records (performance)', async () => {
            ...
            // Create 10 members
            const members = await Promise.all(Array.from({ length: 10 }, () => factories.createTestMember(testChurch.id)));
    ...
    describe('getMessageHistory()', () => {
        ...
        test('✅ Should filter by status', async () => {
            ...
            const pendingResult = await messageService.getMessageHistory(testChurch.id, { status: 'pending' });
    ...
    describe('getMessageDetails()', () => {
        ...
        test('✅ Should throw error for nonexistent message', async () => {
            await expect(messageService.getMessageDetails('nonexistent')).rejects.toThrow('Message not found');
        });
    });
    ...
    describe('updateRecipientStatus()', () => {
        test('✅ Should update recipient to delivered', async () => {
            ...
            if (recipient) {
                await messageService.updateRecipientStatus(recipient.id, 'delivered', message.id);
        ...
        test('✅ Should update recipient to failed with reason', async () => {
            ...
            if (recipient) {
                await messageService.updateRecipientStatus(recipient.id, 'failed', message.id, { failureReason: 'Number not in service' });
                const updated = await prisma.messageRecipient.findUnique({
                    where: { id: recipient.id },
                });
                expect(updated?.status).toBe('failed');
    ...
    describe('Error Handling', () => {
        test('✅ Should handle database errors gracefully', async () => {
            await expect(messageService.resolveRecipients('invalid-church', {
                targetType: 'all',
            })).resolves.toEqual([]);
        });
    });
