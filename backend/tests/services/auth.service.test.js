...
describe('AuthService - registerChurch', () => {
    ...
    describe('successful registration', () => {
        test('✅ Should create church with valid input', async () => {
            ...
            stripeService.createCustomer.mockResolvedValue(mockStripeId);
        ...
        test('✅ Should hash password with bcrypt (not plain text)', async () => {
            ...
            stripeService.createCustomer.mockResolvedValue(mockStripeId);
            jwtUtils.generateAccessToken.mockReturnValue('access_token_124');
            jwtUtils.generateRefreshToken.mockReturnValue('refresh_token_124');
            const input = {
        ...
        test('✅ Should create Stripe customer', async () => {
            ...
            stripeService.createCustomer.mockResolvedValue(mockStripeId);
            jwtUtils.generateAccessToken.mockReturnValue('access_token_125');
            jwtUtils.generateRefreshToken.mockReturnValue('refresh_token_125');
            const input = {
            ...
            expect(stripeService.createCustomer).toHaveBeenCalledWith(input.email, input.churchName);
        });
        test('✅ Should set trial to 14 days from now', async () => {
            const mockStripeId = 'cus_test_126';
            stripeService.createCustomer.mockResolvedValue(mockStripeId);
            jwtUtils.generateAccessToken.mockReturnValue('access_token_126');
            jwtUtils.generateRefreshToken.mockReturnValue('refresh_token_126');
            const input = {
    ...
    describe('error cases', () => {
        test('❌ Should reject duplicate email', async () => {
            ...
            stripeService.createCustomer.mockResolvedValue(mockStripeId1);
            jwtUtils.generateAccessToken.mockReturnValue('access_token_201');
            jwtUtils.generateRefreshToken.mockReturnValue('refresh_token_201');
            const input = {
            ...
            stripeService.createCustomer.mockResolvedValue(mockStripeId2);
            jwtUtils.generateAccessToken.mockReturnValue('access_token_202');
            jwtUtils.generateRefreshToken.mockReturnValue('refresh_token_202');
        ...
        test('❌ Should reject weak passwords', async () => {
            ...
            for (const weakPassword of weakPasswords) {
                ...
                stripeService.createCustomer.mockResolvedValue(mockStripeId);
                jwtUtils.generateAccessToken.mockReturnValue('access_token');
                jwtUtils.generateRefreshToken.mockReturnValue('refresh_token');
                const input = {
