...
import { recordSMSUsage, getSMSUsageSummary, calculateBatchCost, getSMSPricing, getCurrentPlan, getPlanLimits, getUsage, isOnTrial, invalidateBillingCache, } from '../../src/services/billing.service.js';
let prisma;
let factories;
describe('Billing Service Unit Tests', () => {
    ...
    describe('getPlanLimits()', () => {
        ...
        test('✅ Should have increasing limits across plans', () => {
            ...
            if (growthLimits && starterLimits) {
                expect(growthLimits.messagesPerMonth).toBeGreaterThanOrEqual(starterLimits.messagesPerMonth);
            }
        });
    });
    // ========== getUsage() Tests ==========
    ...
    describe('invalidateBillingCache()', () => {
        ...
        test('✅ Should handle non-existent church cache gracefully', async () => {
            ...
            await expect(invalidateBillingCache('non-existent-id')).resolves.not.toThrow();
        });
        test('✅ Should be callable without awaiting (async)', () => {
            const testChurch = factories.createTestChurch().then((c) => c.id);
