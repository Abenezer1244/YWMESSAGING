...
export function generateTestToken(
  userId: string,
  churchId: string,
  secret: string = process.env.JWT_SECRET || 'test-secret-min-32-chars-required-!!',
  expiresIn: string = '1h'
): string {
  return jwt.sign(
    {
...
export function generateTestRefreshToken(
  userId: string,
  churchId: string,
  secret: string = process.env.JWT_REFRESH_SECRET || 'test-refresh-secret-min-32-chars-!!',
  expiresIn: string = '7d'
): string {
  return jwt.sign(
    {
 ...
 */
export function verifyTestToken(token: string, secret: string = process.env.JWT_SECRET || 'test-secret-min-32-chars-required-!!'): any {
  try {
 ...
 */
export function createStripeWebhookSignature(payload: string, secret: string = 'whsec_test'): string {
 ...
 */
export function createTelnyxWebhookSignature(payload: string, secret: string = 'test_secret'): string {
  const crypto = require('crypto');
  const hmac = crypto.createHmac('sha256', secret);
  hmac.update(payload);
