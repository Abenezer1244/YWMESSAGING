 ...
 * Get plan by name
 */
export function getPlan(planName: PlanName): PlanLimits {
 ...
 */
export function getAllPlans(): Array<PlanLimits & { id: PlanName }> {
...
export function checkLimit(
  planName: PlanName,
  limitType: 'branches' | 'members' | 'messagesPerMonth' | 'coAdmins',
  currentCount: number
): boolean {
 ...
 */
export function getPlanFromSubscription(subscriptionPlan?: string): PlanName {
  if (subscriptionPlan && subscriptionPlan in PLANS) {
    return subscriptionPlan as PlanName;
  }
  return 'starter';
}
