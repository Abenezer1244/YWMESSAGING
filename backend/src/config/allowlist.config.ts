 ...
 */
export function isWebhookAllowlisted(provider: string): boolean {
  ...
  return allowlistConfig.webhooks.some(
    (allowedProvider) =>
      allowedProvider.toLowerCase() === normalizedProvider ||
      allowedProvider === '*'
  );
}
 ...
 */
export function isServiceAllowlisted(serviceName: string): boolean {
  if (!serviceName) return false;

  // Normalize to lowercase for comparison
  const normalizedService = serviceName.toLowerCase();

  return allowlistConfig.services.some(
    (allowedService) =>
      allowedService.toLowerCase() === normalizedService ||
      allowedService === '*'
  );
}
 ...
 */
export function isIPAllowlisted(ipAddress: string): boolean {
