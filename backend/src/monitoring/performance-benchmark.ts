...
export class PerformanceBenchmark {
  ...
  constructor(baselineDir: string = BASELINE_DIR) {
   ...
   */
  recordMetrics(metrics: Array<{ name: string; duration: number; unit?: 'ms' | 'sec' | '%' }>) {
    metrics.forEach((m) => {
   ...
   */
  createBaseline(name: string, version: string = '1.0.0'): BenchmarkBaseline {
   ...
   * Save baseline to file
   */
  private saveBaseline(baseline: BenchmarkBaseline) {
  ...
  analyzeRegression(
    ...
    name: string = `regression-${Date.now()}`
  ): RegressionAnalysis {
   ...
   */
  printRegressionReport(analysis: RegressionAnalysis) {
