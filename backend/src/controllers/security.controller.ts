...
export async function getDashboardHandler(req: Request, res: Response) {
  try {
    ...
    const alertsBySeverity = {
      critical: alerts24h.filter((a: any) => a.severity === 'CRITICAL').length,
      high: alerts24h.filter((a: any) => a.severity === 'HIGH').length,
      medium: alerts24h.filter((a: any) => a.severity === 'MEDIUM').length,
      low: alerts24h.filter((a: any) => a.severity === 'LOW').length,
    };
