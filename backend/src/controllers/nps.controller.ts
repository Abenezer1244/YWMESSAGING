...
export async function submitSurvey(req: Request, res: Response) {
  try {
    ...
    // Submit survey
    const survey = await npsService.submitNPSSurvey(
      churchId,
      responderId,
      input
    );
    ...
    res.status(500).json({
      success: false,
      error: (error as Error).message || 'Failed to submit survey',
    });
  }
}
 ...
 */
export async function getRecentSurveys(req: Request, res: Response) {
  try {
    const churchId = req.user?.churchId;
    ...
    const surveys = await npsService.getRecentSurveys(
      churchId,
      limit,
      offset
    );
    ...
    res.status(500).json({
      success: false,
      error: (error as Error).message || 'Failed to fetch surveys',
    });
  }
}
 ...
 */
export async function getNPSByCategory(req: Request, res: Response) {
  try {
    const churchId = req.user?.churchId;
    ...
    const categoryScores = await npsService.getNPSByCategory(
      churchId,
      daysBack
    );
    ...
    res.status(500).json({
      success: false,
      error: (error as Error).message || 'Failed to fetch category scores',
    });
  }
}
