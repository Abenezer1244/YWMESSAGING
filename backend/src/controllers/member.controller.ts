...
export async function listMembers(req: Request, res: Response) {
  try {
    ...
    const limit = Math.min(10000, req.query.limit ? parseInt(req.query.limit as string) : 25);
    ...
    console.error('[listMembers] ERROR:', (error as Error).message);
    res.status(400).json({
      success: false,
...
export async function updateMember(req: Request, res: Response) {
  try {
    ...
    console.error('[updateMember] ERROR:', (error as Error).message);
...
export async function deleteMember(req: Request, res: Response) {
  try {
    ...
    console.error('[deleteMember] ERROR:', (error as Error).message);
    const statusCode = (error as Error).message.includes('not found') ? 404 : 400;
    res.status(statusCode).json({
      success: false,
      error: (error as Error).message,
...
export async function importMembers(req: Request, res: Response) {
  try {
    ...
    console.error('[importMembers] ERROR:', (error as Error).message);
    res.status(400).json({
      success: false,
      error: (error as Error).message,
    });
