...
export interface RateLimitConfig {
  capacity: number; // Max tokens (e.g., 100 messages)
  windowSeconds: number; // Refill window (e.g., 3600 = 1 hour)
...
export async function checkRateLimit(
  ...
  cost: number = config.costPerRequest || 1
): Promise<boolean> {
  try {
    ...
    let bucket: any = data ? JSON.parse(data) : null;
 ...
 */
export async function resetUserBucket(userId: string): Promise<void> {
  try {
    const key = `rate_limit:${userId}`;
 ...
 */
export async function getViolationHistory(userId: string): Promise<any[]> {
  try {
    const key = `rate_limit:violations:${userId}`;
 ...
 */
export async function cleanupExpiredBuckets(): Promise<void> {
  try {
