...
export function parseCSV(fileBuffer: Buffer): MemberCSVRow[] {
  ...
  const result = Papa.parse<MemberCSVRow>(csvString, {
    ...
    transformHeader: (h) => {
      ...
      if (lower === 'firstname' || lower === 'first name' || lower === 'first_name') return 'firstName';
      if (lower === 'lastname' || lower === 'last name' || lower === 'last_name') return 'lastName';
      if (lower === 'phone' || lower === 'phone number') return 'phone';
      if (lower === 'email' || lower === 'email address') return 'email';
