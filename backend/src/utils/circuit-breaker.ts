...
interface CircuitBreakerConfig {
  ...
  resetTimeout: number; // Milliseconds before trying HALF_OPEN (default: 60000ms = 1 min)
  halfOpenRequests: number; // Requests allowed in HALF_OPEN state (default: 2)
...
class CircuitBreaker {
  ...
  private halfOpenRequestsAllowed: number;
  private halfOpenRequestsUsed = 0;
  ...
  async execute<T>(callback: () => Promise<T>): Promise<T> {
    ...
    if (this.state === 'HALF_OPEN') {
      if (this.halfOpenRequestsUsed >= this.halfOpenRequestsAllowed) {
        this.metrics.rejectedRequests++;
        throw new Error(
          `Circuit breaker in HALF_OPEN state, request limit exceeded for ${this.config.name}`
        );
      }
  ...
  private onFailure(): void {
    ...
    if (this.state === 'HALF_OPEN') {
      ...
      console.error(
        `🔴 [CircuitBreaker:${this.config.name}] Service failed during recovery, returning to OPEN`
      );
    }
    // If enough failures in CLOSED, go to OPEN
      ...
      this.nextAttemptTime = new Date(Date.now() + this.config.resetTimeout);
      console.error(
        `🔴 [CircuitBreaker:${this.config.name}] Failure threshold reached, circuit OPEN`
      );
    }
  }
