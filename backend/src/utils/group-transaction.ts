 ...
 * - Bulk operations
 */
export async function executeGroupTransaction<T>(
  groupId: string,
  churchId: string,
  operationType: 'add_members' | 'remove_members' | 'update_group' | 'bulk_operation',
  ...
  try {
    ...
    logTransactionError(transactionName, error as Error);

    if (error instanceof Error) {
      if (error.message.includes('Transaction conflict')) {
        throw new DatabaseError(
          'Group operation failed due to concurrent modification. Please retry.',
          { transactionName, cause: 'concurrent_conflict' }
        );
      }

      if (error.message.includes('Transaction timeout')) {
        throw new DatabaseError(
          'Group operation timeout. Please try again.',
          { transactionName, cause: 'timeout' }
        );
      }
    }
 ...
 */
export async function executeBatchMemberImport<T>(
  groupId: string,
  churchId: string,
  callback: (tx: PrismaClient) => Promise<T>
): Promise<T> {
