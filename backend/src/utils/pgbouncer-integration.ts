...
interface MonitoringConfig {
  ...
  thresholds: {
    utilizationWarning: number; // 0-100%
    utilizationCritical: number; // 0-100%
    connectionLeakThreshold: number; // connections
    idleTimeoutWarning: number; // seconds
  };
}

// Singleton monitoring state
let monitoringInterval: NodeJS.Timeout | null = null;
let poolMetricsHistory: PoolMetrics[] = [];
 ...
 * // Later: cleanup();
 */
export function initializePoolMonitoring(
 ...
 */
export async function collectPoolMetrics(): Promise<PoolMetrics> {
 ...
 * }
 */
export function getHealthStatus(): PoolHealthStatus {
  ...
  if (latestMetrics.utilization > DEFAULT_CONFIG.thresholds.utilizationCritical) {
    ...
    recommendations.push(
      `Increase max connection pool size (current: ${latestMetrics.maxConnections})`
    );
  ...
  if (
    latestMetrics.idleConnections > 5 &&
 ...
 */
export function getPoolMetrics(): PoolMetrics & { utilizationTrend: number } {
  if (poolMetricsHistory.length === 0) {
    return {
      timestamp: Date.now(),
      activeConnections: 0,
...
export function getScalingRecommendation(): {
  ...
  const avgUtilization =
    lastTenMetrics.reduce((sum, m) => sum + m.utilization, 0) / lastTenMetrics.length;
 ...
 * @internal
 */
function checkPoolHealth(metrics: PoolMetrics, config: MonitoringConfig): void {
  const warnings: string[] = [];

  if (metrics.utilization > config.thresholds.utilizationCritical) {
    warnings.push(
      `Pool utilization critical: ${metrics.utilization}% (threshold: ${config.thresholds.utilizationCritical}%)`
    );
  } else if (metrics.utilization > config.thresholds.utilizationWarning) {
    warnings.push(
      `Pool utilization warning: ${metrics.utilization}% (threshold: ${config.thresholds.utilizationWarning}%)`
    );
  }
 ...
 * @internal
 */
function getEstimatedActiveConnections(): number {
 ...
 * @internal
 */
export function resetPoolMonitoring(): void {
  poolMetricsHistory = [];
  connectionLeakDetected = false;
  lastPoolSize = 0;
}
...
export function getPoolStatsSummary(): {
  ...
  const avg = {
    activeConnections:
      poolMetricsHistory.reduce((s, m) => s + m.activeConnections, 0) / poolMetricsHistory.length,
    idleConnections:
      poolMetricsHistory.reduce((s, m) => s + m.idleConnections, 0) / poolMetricsHistory.length,
    totalConnections:
      poolMetricsHistory.reduce((s, m) => s + m.totalConnections, 0) / poolMetricsHistory.length,
    utilization:
      poolMetricsHistory.reduce((s, m) => s + m.utilization, 0) / poolMetricsHistory.length,
  };
