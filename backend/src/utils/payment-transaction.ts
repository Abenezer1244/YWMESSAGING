 ...
 */
export async function executePaymentTransaction<T>(
  paymentId: string,
  churchId: string,
    ...
    if (error instanceof Error) {
      if (error.message.includes('Transaction conflict')) {
        throw new DatabaseError(
          'Payment processing failed due to concurrent request. Please retry.',
 ...
 */
export async function executeSubscriptionTransaction<T>(
