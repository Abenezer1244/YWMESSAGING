...
interface ArchiveConfig {
  months?: number; // Archive partitions older than X months
  keepMonths?: number; // Keep minimum X months of data
  outputBucket?: string; // S3 bucket for archive
...
interface PartitionHealth {
  ...
  details: {
    hasGaps: boolean;
    largestPartition: { name: string; sizeBytes: number };
    smallestPartition: { name: string; sizeBytes: number };
    sizeVariance: number; // Standard deviation
  };
}
 ...
 */
export async function getPartitionInfo(tableName: string): Promise<PartitionInfo> {
 ...
 * }
 */
export async function validatePartitions(tableName: string): Promise<PartitionHealth> {
  ...
  if (sizeVariance > 50) {
    ...
    recommendations.push(
      `Partition size variance high (${sizeVariance.toFixed(1)}%). Consider rebalancing.`
    );
  }
  ...
  if (oldestDate) {
    ...
    if (ageMonths > 24) {
      recommendations.push(
        `Data older than 24 months detected (${ageMonths} months). Consider archival.`
      );
    }
  }
 ...
 */
export async function getPartitionMetrics(tableName: string): Promise<PartitionMetrics> {
  const info = await getPartitionInfo(tableName);
...
export async function planCleanup(
  ...
  if (toArchive.length > 0) {
    recommendations.push(
      `Archive ${toArchive.length} partitions (${(archiveSize / 1024 / 1024 / 1024).toFixed(2)}GB)`
    );
 ...
 */
export function generatePartitioningSQL(
  tableName: string,
  partitioningKeyColumn: string,
 ...
 */
export function estimatePartitioningSavings(
  tableName: string,
  currentIndexSizeMb: number
): {
 ...
 * @param tableName - Name of partitioned table
 */
export async function logPartitionStats(tableName: string): Promise<void> {
