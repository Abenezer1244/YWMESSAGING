...
let readClients: Map<string, PrismaClient> = new Map();
let replicaHealth: Map<string, ReplicaHealthStatus> = new Map();
 ...
 * @internal
 */
export function recordReplicaFailure(replicaUrl: string, error: Error): void {
 ...
 * @internal
 */
export function recordReplicaSuccess(replicaUrl: string): void {
  const status = replicaHealth.get(replicaUrl);
  if (!status) {
    return;
  }
 ...
 * }
 */
export async function checkReplicaHealth(): Promise<ReplicaStatus> {
  ...
  if (primaryClient) {
    try {
      ...
      logger.error('Primary database unhealthy', error as Error);
    }
  }
  ...
  for (const [replicaUrl, client] of readClients.entries()) {
    try {
      ...
      recordReplicaFailure(replicaUrl, error as Error);
    }
  }
...
export async function disconnectAllClients(): Promise<void> {
  try {
    ...
    logger.error('Error disconnecting clients', error as Error);
  }
}
...
function startHealthMonitoring(): void {
  ...
  setInterval(async () => {
    try {
      ...
      logger.debug('Health check error', { error: (error as Error).message });
    }
