...
interface RateLimiterInstance {
  ...
  getGlobalStats: () => Promise<{ totalRequests: number; totalBlocks: number }>;
...
export function createRateLimiter(
   ...
   */
  async function check(key: string): Promise<RateLimitStatus> {
  ...
  async function reset(key: string): Promise<void> {
    try {
      ...
      logger.error(`Failed to reset rate limit for ${key}`, error as Error);
    }
  }
   ...
   */
  async function getAnalytics(key: string): Promise<RateLimitAnalytics> {
    try {
      ...
      logger.error(`Failed to get analytics for ${key}`, error as Error);
      return {
        apiKey: key,
        requestsUsed: 0,
        requestLimit: maxRequests,
   ...
   */
  async function getGlobalStats(): Promise<{ totalRequests: number; totalBlocks: number }> {
    try {
      // This would scan all rate limit keys and aggregate
      // For now, return placeholder
      return { totalRequests: 0, totalBlocks: 0 };
    } catch (error) {
      logger.error('Failed to get global rate limit stats', error as Error);
      return { totalRequests: 0, totalBlocks: 0 };
    }
  }
   ...
   */
  function updateConfig(newConfig: Partial<RateLimitConfig>): void {
 ...
 * app.get('/api/endpoint', enforceRateLimit(limiter), handler);
 */
export function enforceRateLimit(limiter: RateLimiterInstance) {
 ...
 * });
 */
export function createApiKeyLimiter(keyConfigs: KeyConfig): RateLimiterInstance {
 ...
 */
export function createTieredLimiter(): RateLimiterInstance {
  const tieredConfigs: KeyConfig = {
