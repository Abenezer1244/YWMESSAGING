 ...
 */
export function calculatePoolSize(options: PoolSizeOptions): { min: number; max: number } {
 ...
 */
export function generatePgbouncerIni(config: PoolConfig): string {
 ...
 */
export function generateDockerCompose(config: PoolConfig, postgresHost: string): string {
  return `
  pgbouncer:
...
export function validatePoolHealth(
  activeConnections: number,
  maxPoolSize: number,
  idleConnections: number = 0,
  waitingConnections: number = 0
): PoolHealth {
...
export function getConnectionString(
  ...
  username: string,
  password: string,
  options: { connectionLimit?: number; poolTimeout?: number } = {}
): string {
 ...
 */
export function logPoolConfiguration(config: PoolConfig): void {
