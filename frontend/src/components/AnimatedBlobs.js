...
export function AnimatedBlobs({ variant = 'default', className = '' }) {
    ...
    return (_jsx("div", { className: `fixed inset-0 overflow-hidden pointer-events-none ${className}`, children: blobsToRender.map((blob) => (_jsx("div", { className: `absolute rounded-full blur-3xl ${blob.size}`, style: {
