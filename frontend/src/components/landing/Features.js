...
export default function Features() {
    ...
    return (_jsxs("section", { id: "features", className: "relative py-20 px-4 sm:px-6 lg:px-8 bg-white dark:bg-slate-950 overflow-hidden transition-colors duration-normal", children: [_jsxs("div", { className: "absolute inset-0 overflow-hidden pointer-events-none", children: [_jsx(motion.div, { className: "absolute top-20 left-1/4 w-80 h-80 bg-accent-400 opacity-10 rounded-full blur-3xl", animate: {
                            y: [0, -30, 0],
                            x: [0, 15, 0],
                        }, transition: {
                            duration: 8,
                            repeat: Infinity,
                            ease: 'easeInOut',
                        } }), _jsx(motion.div, { className: "absolute bottom-20 right-1/4 w-96 h-96 bg-accent-500 opacity-15 rounded-full blur-3xl", animate: {
                            y: [0, 20, 0],
                            x: [0, -20, 0],
                        }, transition: {
                            duration: 10,
                            ...
                            delay: 1,
                                        ...
                                        }, children: "Stay Connected" })] }), _jsx(motion.p, { className: "text-lg text-slate-700 dark:text-slate-300 max-w-3xl mx-auto font-light leading-relaxed", initial: { opacity: 0 }, whileInView: { opacity: 1 }, transition: { duration: 0.8, delay: 0.2 }, viewport: { once: true }, children: "Powerful features designed specifically for churches managing multiple locations and hundreds of members." })] }), _jsx(motion.div, { className: "grid md:grid-cols-2 lg:grid-cols-3 gap-6", variants: containerVariants, initial: "hidden", whileInView: "visible", viewport: { once: true, margin: '-100px' }, children: features.map((feature, index) => (_jsx(motion.div, { variants: itemVariants, children: _jsxs(Card, { variant: "default", className: "group relative p-8 bg-slate-100/50 dark:bg-slate-900/50 border border-slate-300/50 dark:border-slate-700/50 hover:border-accent-400/50 backdrop-blur-xl rounded-lg overflow-hidden h-full", children: [_jsx(motion.div, { whileHover: {
                                            scale: 1.02,
                                            backgroundColor: 'rgba(15, 23, 42, 0.8)',
                                            borderColor: 'rgba(234, 179, 8, 0.5)',
                                        }, transition: { duration: 0.3 }, className: "absolute inset-0 rounded-lg" }), _jsx(motion.div, { className: "absolute inset-0 bg-gradient-to-r from-accent-500/0 via-accent-400/0 to-accent-300/0 group-hover:from-accent-500/5 group-hover:via-accent-400/5 group-hover:to-accent-300/5 rounded-lg pointer-events-none", animate: {
                                            opacity: [0, 0.3, 0],
                                        }, transition: {
                                            duration: 3,
                                            repeat: Infinity,
                                            delay: index * 0.3,
                                        } }), _jsxs(motion.div, { className: "relative z-10", initial: { opacity: 0 }, whileInView: { opacity: 1 }, transition: { duration: 0.6, delay: index * 0.1 + 0.2 }, viewport: { once: true }, children: [_jsx(motion.div, { className: "w-12 h-12 bg-accent-500 text-slate-950 rounded-lg flex items-center justify-center mb-5 group-hover:shadow-lg group-hover:shadow-accent-500/50", whileHover: {
                                                    scale: 1.15,
                                                    rotate: [0, -10, 10, -5, 5, 0],
                                                }, transition: { type: 'spring', stiffness: 300 }, children: feature.icon }), _jsx("h3", { className: "text-lg font-semibold text-slate-900 dark:text-white mb-3", children: feature.title }), _jsx("p", { className: "text-slate-700 dark:text-slate-300 text-sm leading-relaxed", children: feature.description })] })] }) }, index))) })] })] }));
}
