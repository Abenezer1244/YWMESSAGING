...
const Input = React.forwardRef(({ label, error, helperText, icon, iconPosition = 'left', maxLength, showCharCount = false, isPassword = false, className = '', value, ...props }, ref) => {
    ...
    return (_jsxs("div", { className: "w-full", children: [label && (_jsxs("label", { className: "block text-sm font-medium text-foreground mb-2", children: [label, props.required && _jsx("span", { className: "text-destructive ml-1", children: "*" })] })), _jsxs("div", { className: "relative", children: [icon && iconPosition === 'left' && (_jsx("div", { className: "absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground flex items-center", children: icon })), _jsx("input", { ref: ref, type: inputType, value: value, maxLength: maxLength, className: combinedInputClassName, ...props }), isPassword && (_jsx("button", { type: "button", onClick: () => setShowPassword(!showPassword), className: "absolute right-3 top-1/2 transform -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors", children: showPassword ? '👁️' : '👁️‍🗨️' })), icon && iconPosition === 'right' && !isPassword && (_jsx("div", { className: "absolute right-3 top-1/2 transform -translate-y-1/2 text-muted-foreground flex items-center", children: icon }))] }), showCharCount && maxLength && (_jsxs("div", { className: "text-xs text-muted-foreground mt-1", children: [charCount, " / ", maxLength] })), error && (_jsxs("p", { className: "text-xs text-destructive mt-1.5 flex items-center gap-1", children: [_jsx("span", { children: "\u26A0\uFE0F" }), error] })), helperText && !error && (_jsx("p", { className: "text-xs text-muted-foreground mt-1.5", children: helperText }))] }));
});
