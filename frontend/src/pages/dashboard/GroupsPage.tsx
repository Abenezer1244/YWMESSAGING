    ...
    <div className="min-h-screen bg-white dark:bg-slate-950 p-6 transition-colors duration-normal">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-4xl font-bold text-slate-900 dark:text-white mb-2">👥 Groups</h1>
            <p className="text-slate-700 dark:text-slate-300">{groups.length} groups in this branch</p>
          </div>
          <Button
            variant="primary"
            size="lg"
        ...
        {groups.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {groups.map((group) => (
              <Card key={group.id} variant="default" className="hover:shadow-lg transition-shadow bg-slate-50 dark:bg-slate-900/50 border-slate-300 dark:border-slate-700">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex-1">
                    <h3 className="text-lg font-semibold text-slate-900 dark:text-white">{group.name}</h3>
                    {group.description && (
                      <p className="text-slate-700 dark:text-slate-300 text-sm mt-1">{group.description}</p>
                    )}
                  </div>
                ...
                <div className="space-y-2 mb-6 pb-6 border-b border-slate-300 dark:border-slate-700">
                  ...
                  {group.welcomeMessageEnabled && (
                    <div>
                      <p className="text-slate-600 dark:text-slate-400 text-sm">Welcome Message</p>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <Card variant="default" className="max-w-sm w-full bg-slate-50 dark:bg-slate-900/50 border-slate-300 dark:border-slate-700">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">⚠️ Delete Group?</h3>
