...
test.describe('User Signup Flow', () => {
  ...
  test('should validate password strength', async () => {
    ...
    await expect(
      page.locator('text=/password must be|at least 12|uppercase|number|special character/i')
    ).toBeVisible();
  });
  ...
  test('should show error for duplicate email', async () => {
    ...
    // Should show error
    await expect(page.locator('text=/already exists|already registered|duplicate|in use/i')).toBeVisible(
      { timeout: 5000 }
    );
  });
